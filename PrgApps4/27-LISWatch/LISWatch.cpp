...
static void CalcWndText(HWND hwnd, PTSTR szBuf, int nLen) {
   ...
   wsprintf(szBufT, TEXT("0x%08X [%s] %s "), hwnd, (PTSTR) szClass,
...
void Dlg_OnTimer(HWND hwnd, UINT id) {
   ...
   HWND hTop = GetTopWindow(hDesk);
   CalcWndText(hTop, szBuf, chDIMOF(szBuf));

   TCHAR szTemp[16] = {0};
   _stprintf(szTemp, TEXT("\r\n0x%08X"), hTop);
   _tcscat(szBuf, szTemp);

   SetWindowText(GetDlgItem(hwnd, IDC_NOTEPAD), szBuf);
   //TRACE(TEXT("top, %s\n"), szBuf);
   //
